...
namespace pk::tools
{
    namespace
    {
        using primitive_mesh_creator = void(*)(scene& scene, primitive_init_info& info);

        void create_plane(scene& scene, primitive_init_info& info);
        void create_cube(scene& scene, primitive_init_info& info);
        void create_uv_sphere(scene& scene, primitive_init_info& info);
        void create_ico_sphere(scene& scene, primitive_init_info& info);
        void create_cylinder(scene& scene, primitive_init_info& info);
        void create_capsule(scene& scene, primitive_init_info& info);
                          ...
                          math::v2 u_range = math::v2 { 0.0f, 1.0f }, math::v2 v_range = math::v2 { 0.0f, 1.0f })
        {
            ...
            for (int j { 0 }; j <= vertical_count; ++j)
            {
                for (int i { 0 }; i <= horizontal_count; ++i)
                {
                    ...
                    math::v2 uv { u_range.x, 1.f - v_range.y };
                    uv.x += i * u_step;
                    uv.y -= j * v_step;
                    uvs.emplace_back(uv);
                }
            ...
            assert(m.positions.size() == (static_cast<u64>(horizontal_count)+1)*(static_cast<u64>(vertical_count)+1));
            ...
            m.uv_sets.resize(1);
            m.uv_sets[0].reserve(num_indices);
